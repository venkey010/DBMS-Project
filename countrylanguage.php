...
$username = $_POST['languages'];
	...
	if(mysqli_num_rows($result)>0){
		...
		while($row = mysqli_fetch_assoc($result)){
			if($username==$row['Language'] || $username=='ALL'){
...
$username = $_POST['languages'];
    ...
    //$city1 = 'call language()';
    $sql='select country.Name as countryName,Language,IsOfficial from country,countrylanguage where country.Code=CountryCode ';
    echo $city;
    ...
    if(mysqli_num_rows($result)>0){
        ...
        while($row = mysqli_fetch_assoc($result)){
            ...
            echo "<tr><td>".$row['country.Name']."</td>  <td>".$row['Language']."</td>  <td>".$row['IsOfficial']."</td>  <td>".$row['Percentage']."</td></tr>";
        }
    }
    }
}
